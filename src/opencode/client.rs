...
/// The `/api` transcript page size (the server accepts 1..=200) and the hard
/// stop for its cursor follow: a misbehaving server must not spin the client
/// forever. 100 pages of 200 is 20k messages, far past any real Session.
const TRANSCRIPT_PAGE_SIZE: usize = 200;
const MAX_TRANSCRIPT_PAGES: usize = 100;
...
impl Client {
    ...
    /// Fetch one session's Session Transcript. The adapter selects the wire
    /// generation internally (ADR-0053): the current `/api` (V2) read is
    /// preferred, and the legacy unprefixed read serves what it cannot.
    ///
    /// The `/api` message store is fed by the V2 execution path, so a Session
    /// driven through the legacy prompt route has no rows there; an empty
    /// first page therefore reads as "this generation cannot serve the read"
    /// and the legacy read takes over. A server without the route (404) falls
    /// back the same way; any other failure surfaces as an error — never a
    /// silent fallback. Prompts stay on the legacy route while it remains
    /// mounted.
    pub async fn transcript(&self, session_id: &str) -> crate::error::Result<SessionTranscript> {
        if let Some(transcript) = self.transcript_v2(session_id).await? {
            return Ok(transcript);
        }
        self.transcript_legacy(session_id).await
    }

    /// The legacy (unprefixed) generation's read: one
    /// `GET /session/{id}/message` response carrying every message, decoded
    /// through the legacy wire decoder.
    async fn transcript_legacy(&self, session_id: &str) -> crate::error::Result<SessionTranscript> {
        let resp = self
            .http()
    ...
    /// The `/api` (V2) generation's read: `GET /api/session/{id}/message`
    /// pages, oldest first, following `cursor.next` to the end.
    ///
    /// `Ok(None)` means this generation cannot serve the read — the route is
    /// not mounted (404), or its store has no rows for the Session (an empty
    /// first page). Any other failure is a real error.
    async fn transcript_v2(&self, session_id: &str) -> crate::error::Result<Option<SessionTranscript>> {
        let Some(first) = self.v2_page(session_id, None).await? else {
            return Ok(None);
        };
        ...
        while let Some(next) = cursor {
            if pages >= MAX_TRANSCRIPT_PAGES {
                tracing::warn!(
                    "transcript v2: cursor still present after {MAX_TRANSCRIPT_PAGES} pages; \
                     returning the {} messages fetched so far",
                    messages.len()
                );
                break;
            }
            let Some(page) = self.v2_page(session_id, Some(&next)).await? else {
                // The route vanished mid-drain (the Session was deleted, or
                // the server was replaced): keep what was already read rather
                // than fail a live Turn's history.
                tracing::warn!("transcript v2: the /api message route vanished mid-drain");
                break;
            };
        ...
        Ok(Some(SessionTranscript::new(messages)))
    }

    /// One `/api` (V2) message page. `Ok(None)` when the route is not mounted
    /// (404), so the caller can fall back to the legacy generation. The first
    /// page is requested oldest-first; a follow-up request carries only the
    /// opaque cursor the previous page returned (the server rejects a cursor
    /// combined with `order`).
    async fn v2_page(
        &self,
        session_id: &str,
        ...
        super::wire::decode_page(&body).map(Some)
    }
...
mod wire_tests {
    ...
    /// The legacy side of the parity pair: every field the `/api` generation
    /// can carry too (no `total` token, which `/api` never reports, and no
    /// `step-start`, which has no `/api` counterpart), so a correct V2
    /// decoder produces the SAME typed views.
    fn legacy_parity_fixture() -> serde_json::Value {
        serde_json::json!([
            {
                "info": {"id": "msg_u1", "role": "user", "time": {"created": 1000}},
            ...
            },
            {
                "info": {
                    ...
                    "modelID": "deepseek-v4-flash",
                    "providerID": "opencode-go",
                    "tokens": {"input": 220, "output": 129,
                               "cache": {"read": 161664, "write": 0}}
                },
                "parts": [
                    {"type": "reasoning", "text": "想一下", "time": {"start": 1110}},
                    {"type": "text", "text": "答案一"},
                    {"type": "tool", "tool": "bash", "callID": "call_1",
                     "state": {"status": "completed", "input": {"command": "ls"},
                    ...
                    {"type": "tool", "tool": "edit", "callID": "call_2",
                     "state": {"status": "completed", "input": {"filePath": "a.rs"},
                               ...
                               "metadata": {"diff": "@@ -1 +1 @@"}}},
                    {"type": "tool", "tool": "mystery", "callID": "call_3",
                     "state": {"status": "weird", "input": {"x": 1}}},
                    {"type": "mystery-part", "payload": 42},
                    {"type": "patch", "hash": "after", "files": ["src/a.rs"]},
                    {"type": "step-finish", "reason": "tool-calls"}
                ]
            },
            {
                ...
                "parts": [{"type": "text", "text": "答案二"}]
            }
        ])
    }

    /// The `/api` equivalent of [`legacy_parity_fixture`]: the same messages
    /// in the current generation's shapes (message `type`, `content` items,
    /// `snapshot`/`finish`, tool `id`/`time.ran`/`structured`, no token
    /// `total`).
    fn v2_parity_page() -> serde_json::Value {
        serde_json::json!({
            "data": [
                {"type": "user", "id": "msg_u1", "time": {"created": 1000}, "text": "问题"},
                {
                    ...
                    "time": {"created": 1100, "completed": 1400},
                    "model": {"id": "deepseek-v4-flash", "providerID": "opencode-go"},
                    "tokens": {"input": 220, "output": 129, "reasoning": 7,
                               "cache": {"read": 161664, "write": 0}},
                    "content": [
                        ...
                        {"type": "tool", "id": "call_2", "name": "edit",
                         "state": {"status": "completed", "input": {"filePath": "a.rs"},
                                   "content": [{"type": "text", "text": "Edit applied successfully."}],
                                   "structured": {"files": [{"file": "a.rs", "patch": "@@ -1 +1 @@"}]}}},
                        {"type": "tool", "id": "call_3", "name": "mystery",
                         "state": {"status": "weird", "input": {"x": 1}}},
                        {"type": "mystery-part", "payload": 42}
                    ],
                    "snapshot": {"start": "before", "end": "after", "files": ["src/a.rs"]},
                    "finish": "tool-calls"
                },
                {
                    "type": "assistant",
    ...
    /// The adapter tries the current `/api` route first; when the server does
    /// not mount it, the legacy read serves the Session unchanged.
    #[tokio::test]
    async fn transcript_tries_the_v2_route_then_falls_back_when_absent() {
        let server = TestHttpServer::start().await;
        server.route(
            "GET",
            "/session/ses_1/message",
        ...
        assert_eq!(
            requests.len(),
            2,
            "the current route is tried, then the legacy one"
        );
        assert_eq!(requests[0].method, "GET");
        assert_eq!(requests[0].path, "/api/session/ses_1/message");
        assert_eq!(requests[0].query_param("limit").as_deref(), Some("200"));
        assert_eq!(requests[0].query_param("order").as_deref(), Some("asc"));
        assert_eq!(requests[1].method, "GET");
        assert_eq!(requests[1].path, "/session/ses_1/message");
    }
    ...
    async fn transcript_prefers_the_v2_route_and_drains_its_cursor() {
        ...
        server.route_sequence(
            "GET",
            "/api/session/ses_v2/message",
            vec![
                MockResponse::json(
                    serde_json::json!({
                        "data": [
                            {"type": "user", "id": "msg_u1", "time": {"created": 1000},
                             "text": "第一个问题"}
                        ],
                    ...
                    serde_json::json!({
                        "data": [
                            {"type": "assistant", "id": "msg_a1",
                             ...
                             "model": {"id": "deepseek-v4-flash", "providerID": "opencode-go"},
                             "content": [{"type": "text", "id": "prt_t", "text": "答案一"}],
                             "finish": "stop"}
                        ],
                        "cursor": {"previous": "p2", "next": "c2"}
                    })
                    .to_string(),
                ),
                MockResponse::json(
                    serde_json::json!({
                        "data": [
                            {"type": "assistant", "id": "msg_a2", "time": {"created": 1300},
                             "model": {"id": "deepseek-v4-flash", "providerID": "opencode-go"},
                             "content": [{"type": "text", "id": "prt_t2", "text": "答案二"}]}
                        ],
        ...
        // A legacy route the read must not touch.
        server.route(
            "GET",
            "/session/ses_v2/message",
            500,
            r#"{"error":"the legacy read must not run"}"#,
        );
        let client = wire_client(&server, None);

        let transcript = client.transcript("ses_v2").await.unwrap();

        let ids: Vec<_> = transcript.messages.iter().map(|m| m.id.as_str()).collect();
        assert_eq!(ids, vec!["msg_u1", "msg_a1", "msg_a2"]);
        let anchor = transcript.newest_user().unwrap().anchor().unwrap();
        let turn = transcript.turn_for_user(&anchor);
        let turn_ids: Vec<_> = turn.messages.iter().map(|m| m.id.as_str()).collect();
        assert_eq!(turn_ids, vec!["msg_a1", "msg_a2"], "the in-flight tail belongs");
        ...
        assert_eq!(requests[0].query_param("limit").as_deref(), Some("200"));
        assert_eq!(requests[0].query_param("order").as_deref(), Some("asc"));
        assert!(requests[0].query_param("cursor").is_none());
        assert_eq!(requests[1].query_param("cursor").as_deref(), Some("c1"));
        assert!(
            requests[1].query_param("order").is_none(),
    ...
    /// for a Session the legacy store knows means "this generation cannot
    /// serve the read", so the legacy decoder answers.
    #[tokio::test]
    async fn transcript_v2_empty_store_falls_back_to_legacy() {
        let server = TestHttpServer::start().await;
        server.route(
            "GET",
            "/api/session/ses_mixed/message",
            200,
            serde_json::json!({"data": [], "cursor": {"previous": null, "next": null}}).to_string(),
        );
        server.route(
            "GET",
            "/session/ses_mixed/message",
            200,
            transcript_fixture().to_string(),
        );
        let client = wire_client(&server, None);

        let transcript = client.transcript("ses_mixed").await.unwrap();

        assert_eq!(transcript.messages.len(), 3, "the legacy read served the Session");
        let requests = server.requests();
        assert_eq!(requests.len(), 2);
        assert_eq!(requests[0].path, "/api/session/ses_mixed/message");
        assert_eq!(requests[1].path, "/session/ses_mixed/message");
    }

    /// A failed `/api` read is a real error, never a silent legacy fallback:
    /// the caller must see the backend's failure.
    #[tokio::test]
    async fn transcript_v2_failure_is_not_masked_by_a_fallback() {
        let server = TestHttpServer::start().await;
        server.route(
            "GET",
            "/api/session/ses_broken/message",
            500,
            r#"{"message":"boom"}"#,
        );
        server.route(
            "GET",
            "/session/ses_broken/message",
            200,
            transcript_fixture().to_string(),
        );
        let client = wire_client(&server, None);

        assert!(client.transcript("ses_broken").await.is_err());
        assert_eq!(server.request_count(), 1, "no fallback after a real failure");
    }

    /// A malformed `/api` envelope is an error, not an empty page that would
    /// masquerade as an unprojected Session.
    #[tokio::test]
    async fn transcript_v2_malformed_page_is_an_error() {
        let server = TestHttpServer::start().await;
        server.route("GET", "/api/session/ses_bad/message", 200, r#"{"data":"nope"}"#);
        server.route(
            "GET",
        ...
        assert!(client.transcript("ses_bad").await.is_err());
        assert_eq!(server.request_count(), 1, "a malformed page is not an empty one");
    }

    /// A server that keeps reporting a next cursor is followed only to the
    /// page cap, then the messages read so far are returned.
    #[tokio::test]
    async fn transcript_v2_cursor_loop_is_bounded() {
        let server = TestHttpServer::start().await;
        server.route_sequence(
            "GET",
            "/api/session/ses_loop/message",
            vec![MockResponse::json(
                serde_json::json!({
                    "data": [
                        {"type": "user", "id": "msg_u1", "time": {"created": 1000}, "text": "hi"}
        ...
        let transcript = client.transcript("ses_loop").await.unwrap();

        assert_eq!(server.request_count(), 100, "the follow stops at the page cap");
        assert_eq!(
            transcript.messages.len(),
            100,
            "the pages read so far are returned"
        );
    }

    /// Parity: the legacy fixture and its `/api` equivalent decode to the
    /// same typed transcript — message for message, payloads included — so
    /// the shared projections and Turn outcome read identically from either
    /// generation. Both sides go through the production adapter (the legacy
    /// one with the current route absent, the `/api` one with the legacy
    /// route unable to serve), which also covers the generation selection.
    #[tokio::test]
    async fn v2_and_legacy_fixtures_yield_the_same_transcript() {
        let legacy_server = TestHttpServer::start().await;
        legacy_server.route(
            "GET",
            "/session/ses_parity/message",
            200,
            legacy_parity_fixture().to_string(),
        );

        let v2_server = TestHttpServer::start().await;
        v2_server.route_sequence(
            "GET",
            "/api/session/ses_parity/message",
            vec![MockResponse::json(v2_parity_page().to_string())],
        );
        v2_server.route(
            "GET",
            "/session/ses_parity/message",
        ...
        // Field for field: parts, raw tool payloads, times, model, tokens,
        // tolerant arms. The fixture carries no token `total`, which `/api`
        // never reports, so even the token view matches exactly.
        assert_eq!(legacy.messages, v2.messages);
        assert_eq!(v2_server.request_count(), 1, "the legacy route is never touched");
        assert_eq!(
            legacy_server.request_count(),
            2,
            "the absent current route falls back"
        );

        // Projections and the Turn outcome agree.
        assert_eq!(
