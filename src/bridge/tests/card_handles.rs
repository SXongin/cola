...
async fn a_split_registers_the_continuation_card() {
    ...
    crate::bridge::turn::flush::flush_card(&app.cards_handle(), "ses_split").await;
...
async fn partial_question_answer_refreshes_the_clicked_card() {
    ...
    crate::bridge::turn::flush::flush_card(&app.cards_handle(), "ses_q").await;
...
async fn a_concurrent_flush_leaves_the_tail_on_one_card() {
    ...
    let first = {
        let app = app.clone();
        tokio::spawn(
            async move { crate::bridge::turn::flush::flush_card(&app.cards_handle(), "ses_split").await },
        )
    };
...
async fn a_resolution_racing_an_in_flight_flush_is_not_resurrected() {
    ...
    let flush = {
        let app = app.clone();
        tokio::spawn(
            async move { crate::bridge::turn::flush::flush_card(&app.cards_handle(), "ses_live").await },
        )
    };
    entered.notified().await;
