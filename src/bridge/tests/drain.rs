...
async fn the_drain_bound_exits_cleanly_and_finishes_the_turn() {
    ...
    // A tiny bound so the long-poll branch runs in milliseconds.
    app.turn_drain_timeout_ms.store(30, Ordering::Relaxed);
    // Group + requester: finalization still sends the completion notice.
    ...
    assert!(
        ...
        "the group completion notice must be unchanged: {:?}",
        platform.calls.lock().await
    );
    assert_no_further_rendering(&backend, &platform).await;
}
