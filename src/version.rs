...
/// Whether this binary is a dev build (ADR-0027): `build.rs` found no clean
/// release tag on HEAD — a branch, a dirty tree, or no git provenance at all.
...
pub fn display_version() -> String {
    render(release_version(), is_dev_build(), BRANCH, SHA, DIRTY.is_some())
}
...
fn render(release: &str, dev: bool, branch: Option<&str>, sha: Option<&str>, dirty: bool) -> String {
    if !dev {
        return format!("cola {release}");
    }
...
mod tests {
    ...
    fn release_renders_bare_version() {
        assert_eq!(
            render("0.7.0", false, Some("main"), Some("ab12cd3"), true),
            "cola 0.7.0"
        );
    }
    ...
    fn dev_renders_branch_at_sha_and_dirty() {
        assert_eq!(
            render("0.7.0", true, Some("feat"), Some("ab12cd3"), true),
    ...
    fn dev_clean_omits_warning() {
        assert_eq!(
            render("0.7.0", true, Some("main"), Some("ab12cd3"), false),
    ...
    fn dev_detached_head_omits_branch() {
        assert_eq!(
            render("0.7.0", true, Some("HEAD"), Some("ab12cd3"), false),
    ...
    fn dev_without_git_shows_no_provenance() {
        assert_eq!(render("0.7.0", true, None, None, false), "cola 0.7.0-dev");
    }

    #[test]
    fn dev_dirty_without_locator_still_flags() {
        assert_eq!(render("0.7.0", true, None, None, true), "cola 0.7.0-dev ⚠");
    }
}
