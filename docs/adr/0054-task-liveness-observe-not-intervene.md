...
- **The read is cheap.** The backend seam already reads one session's
  transcript and live run state (ADR-0053), and cola keeps the call's metadata
  verbatim (`ToolCall.metadata`); no new API surface is needed.
...
- **One liveness line on the running panel.** While a task Tool Panel is live
  it carries the child's current tool, the age of its last activity, and —
  when the child has a pending Permission/Question — the wait state, reusing
  the Card's existing wait vocabulary (等待授权/等待回答).
...
- The task Tool Panel gains a display-only liveness section sourced from the
  call's `sessionId`: the child's transcript/run-state read plus the existing
  pending-wait lists. It opens no new write path, and ADR-0045 already keeps a
  running panel on the live card.
...
several calls. No new canonical term.
