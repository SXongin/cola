...
- A foreign session's state is fully readable with no writes:
  - pending Permission/Question: server `GET /permission?directory=` and `GET /question?directory=` (`src/opencode/client.rs:411-471`), already polled every 3 s by the request poller (`RequestFlow::poll_loop`, `src/bridge/request/flow.rs:410`; the sweep's list/surface loop, `:439-510`).
  ...
  - transcript tail and message times: `GET /session/{id}/message` (already used by `src/bridge/turn/render.rs`); the newest user message's authorship is authoritative via the `msg_cola_` id prefix (ADR-0026).
...
- The busy-follow's accumulator composition with claimed pending blocks needs implementation care in `src/bridge/request/` and `src/bridge/turn/render.rs` (a blocked external run approved from the snapshot then streams the rest); verify against the sweep's vanished-block resolution (`resolve_vanished_inline`, `src/bridge/request/kind.rs:195`) and the `turn_started_ms` replacement guard (`Turn::armed_turn_anchor`, `src/bridge/turn/mod.rs:1029`; the turn filter, `src/bridge/turn/render.rs:428-445`).
