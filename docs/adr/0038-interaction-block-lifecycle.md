...
4. **Resolution leaves an Interaction Receipt.** In place of the block, one markdown line: `✅ 已允许一次 · 14:03`, `✅ 已回答：/a、main`, `🚫 已拒绝`, `⏱ 已由其他客户端处理`, `🔄 已开启自动授权`. No controls, so a late click can never be ambiguous. Applies on the Session Snapshot's claimed blocks too.
