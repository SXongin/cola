...
4. **Resolution leaves an Interaction Receipt, anchored where the interaction happened, and it names its target.** The receipt replaces the block's slot in the accumulator but renders at the **timeline position the block was surfaced at** — right after the content it belongs to, above everything streamed later — never in the card's moving tail (a live block stays in the tail; only its residue is anchored). It carries its target, so it still identifies its request where the anchor cannot: a poll race can surface a block before its tool panel lands, a sub-task child's block has no command panel on the parent card, and a toggle resolves several blocks at once. One markdown line, no controls, so a late click can never be ambiguous: `✅ 已允许一次：⚡ 执行 Shell 命令 \`ls -la\` · 14:03`, `✅ 已回答：目录 /a、分支 main`, `🚫 已拒绝：…`, `⏱ 已由其他客户端处理：…`, `🔄 已开启自动授权：…`. Applies on the Session Snapshot's claimed blocks too.
