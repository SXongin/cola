...
- **Thread = session boundary**: a Feishu topic (`thread_id`) maps to one OpenCode sessionID; a chat's top-level messages map to its lobby session (see ADR 0003). Stored in `~/.cola/sessions.json`.
