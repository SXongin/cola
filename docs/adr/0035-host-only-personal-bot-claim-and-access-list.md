...
4. **Total enforcement**: non-Host Principals are refused — messages get a short denial; card clicks are rejected on the Principal check, not merely hidden (Feishu cannot hide buttons per user).
...
- It closes the exposure found in the grilling: today anyone who can message the bot is treated as the Host.
...
- **A config list of admitted Principals + `/whoami`**: works, but makes the Host transcribe an opaque app-scoped id by hand; rejected.
