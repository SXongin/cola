...
- **Check gates**: only `Format`, `Check`, `Test (macos-latest)`, `Test (windows-latest)` and `Dependency audit` are required by the `main: CI` ruleset — judge merge-readiness with `gh pr checks <branch> --required`, never by waiting for every check. CodeQL is advisory on normal PRs: do not wait for it. The release cut is the exception (`cargo xtask release` watches every check, CodeQL included). `release/*` and docs-only PRs skip the code-dependent jobs — a skipped job reports success and still satisfies the ruleset; `Dependency audit` always runs.
